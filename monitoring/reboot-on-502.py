...
console = Console(force_terminal=IS_TTY, force_interactive=IS_TTY, force_jupyter=False)

# Configure logging
if IS_TTY:
    handler = RichHandler(
        rich_tracebacks=True, console=console, show_path=False, markup=True
    )
...
class MonitorConfig:
    ...
    def _parse_maintenance_windows(windows_str: str) -> List[tuple]:
        ...
        for window in windows_str.split(","):
            ...
            try:
                ...
                windows.append(
                    (dt_time(start_hour, start_min), dt_time(end_hour, end_min))
                )
    ...
    def validate(self) -> None:
        ...
        if IS_TTY:
            ...
            table = Table(
                title="Configuration", show_header=False, border_style="green"
            )
            ...
            table.add_row(
                "Running as", "Interactive TTY" if IS_TTY else "Service/Background"
            )
...
def handle_502_error(
    config: MonitorConfig, error_count: int, error_tracker: ErrorTracker
) -> None:
...
def log_status(status_code: Optional[int]) -> None:
    ...
    if status_code == 200:
        if IS_TTY:
            logger.info(
                f"✓ Status:  [green]{status_code}[/green] (OK)", extra={"markup": True}
            )
        else:
        ...
        if IS_TTY:
            logger.info(
                f"Status: [yellow]{status_code}[/yellow]", extra={"markup": True}
            )
        else:
...
def send_startup_notification(config: MonitorConfig) -> None:
    ...
    features.append(
        f"Error threshold: {config.error_threshold}/{config.error_window_seconds}s"
    )
...
def run_monitor(config: MonitorConfig) -> None:
    ...
    logger.info(
        f"Starting monitoring on {config.url} (interval: {config.check_interval}s)"
    )
    logger.info(
        f"Alert threshold: {config.error_threshold} errors in {config.error_window_seconds}s"
    )

    if config.maintenance_windows:
        logger.info(
            f"Maintenance windows configured: {len(config.maintenance_windows)}"
        )
...
def main() -> None:
    ...
    try:
        ...
        if IS_TTY:
            ...
            logger.critical(
                "Please set required environment variables:  TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID"
            )
        return
